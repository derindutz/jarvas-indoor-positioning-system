...
void DW1000RangingClass::startAsAnchor(char address[], const byte mode[]) {
	...
	//we need to define a random short address:
        ...
        _currentShortAddress[0]=_currentAddress[0]; //random(0,256);
        _currentShortAddress[1]=_currentAddress[1]; //random(0,256);
...
void DW1000RangingClass::setLoc(float locationX, float locationY){
    locX = (uint16_t)(locationX * 100.0f);
	locY = (uint16_t)(locationY * 100.0f);
}
float DW1000RangingClass::getTagX(){
    return ((float)locX)/100.0f;
}

float DW1000RangingClass::getTagY(){
    return ((float)locY)/100.0f;
}
short DW1000RangingClass::detectMessageType(byte datas[]) {
	if(datas[0] == 0xC5) {
		return BLINK;
	}
	else if(datas[0] == 0xA6){
        return LOC;
    }
...
void DW1000RangingClass::loop() {
	...
	if(_receivedAck) {
		...
		int messageType = detectMessageType(data);
		//Serial.print("Type: ");
        //if(_type == ANCHOR) Serial.println(messageType);
		...
		else if(messageType == LOC && _type == ANCHOR){
			// Serial.println("Hi");
			output = ~output;
...
void DW1000RangingClass::transmitLoc() {
    transmitInit();
	_globalMac.generateLocFrame(data, _currentShortAddress, _currentShortAddress);
   	byte bytes[4];
